...
export class ReplaceTopicNamesByI18nKeys1616573753595 implements MigrationInterface {
  ...
  private updateQuery(shortName: string, updatedValue: string): string {
    const from = 'FROM balance_sheet b, rating r';
    const where = `where b.type = 'Compact' and b."ratingId" = r.id and topic."ratingId" = r.id and topic."shortName"='${shortName}'`
    return `UPDATE topic SET name='${updatedValue}' ${from} ${where};`;
  }

  public async up(queryRunner: QueryRunner): Promise<void> {
    const i18nPrefix = 'v5:compact'
    const shortNameToTranslationKeyMap = new Map([
      ['A1', `${i18nPrefix}.A1`],
      ['A2', `${i18nPrefix}.A2`],
      ['A3', `${i18nPrefix}.A3`],
      ['A4', `${i18nPrefix}.A4`],
    ...
    for (let [shortName, translationKey] of shortNameToTranslationKeyMap) {
      await queryRunner.query(this.updateQuery(shortName, translationKey));
    }
  }
