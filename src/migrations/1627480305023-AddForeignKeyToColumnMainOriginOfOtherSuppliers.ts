  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    const queries = [
      `ALTER TABLE "company_facts" ADD CONSTRAINT "REL_company_facts_main_origin_of_other_suppliers" UNIQUE ("mainOriginOfOtherSuppliersId")`,
      `ALTER TABLE "company_facts" ADD CONSTRAINT "FK_company_facts_main_origin_of_other_suppliers" FOREIGN KEY 
        ("mainOriginOfOtherSuppliersId") REFERENCES "main_origin_of_other_suppliers"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    ];
