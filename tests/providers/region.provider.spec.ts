...
describe('Region Provider', () => {
    ...
    it('should contain regions of supplyfractions', async (done) => {
        companyFacts.supplyFractions = [new SupplyFraction(undefined, 'A', 'CRI', 100),
            new SupplyFraction(undefined, 'B', 'DEU', 200)]
        const regionProvider = await RegionProvider.createFromCompanyFacts(companyFacts, regionRepo);
    ...
    it('should contain regions of employeesFraction', async (done) => {
        companyFacts.employeesFractions = [new EmployeesFraction(undefined, 'AFG', 3),
